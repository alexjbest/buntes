...
\item\hypertarget{item-nonsing-pt}{}\(E\) is nonsingular of genus 1 with a distinguished point \(P_0\).%
\item\hypertarget{item-nonsing-gp}{}\(E\) is nonsingular with an algebraic group structure.%
...
\par
\hypertarget{p-69}{}%
For injectivity, show that if the above section has the same values on \(z_1,z_2\) then it is a theta function for some sublattice. Almost all sections aren't theta functions for a sublattice (this uses \hyperref[prop-dimension-sections-complex]{Proposition~\ref{prop-dimension-sections-complex}}).%
\par
\hypertarget{p-70}{}%
...
\item\hypertarget{li-20}{}Given a nonsingular surface \(V,\, P\in V\) then \(\exists \alpha\colon W \to V\) regular that induces an isomorphism \(\alpha\colon W\smallsetminus \alpha^{-1} (P) \to V\smallsetminus P\), but \(\alpha^{-1}(P)\) is a projective line. The rational map represented by \(\alpha^{-1}\) is not regular on \(V\) (where to send \(P\)?).%
\end{enumerate}
%
\end{example}
...
\hypertarget{p-111}{}%
Alternative definition: A line bundle on \(X\) is equivalent to the following data:\leavevmode%
\begin{itemize}[label=\textbullet]
...
\hypertarget{p-126}{}%
\(\sheaf M\) is the pullback of the line bundle of \hyperref[cor-5-2]{Corollary~\ref{cor-5-2}} via the map \((f,g,h) \colon V \to A\times A\times A\).%
\end{proof}
\hypertarget{p-127}{}%
...
\hypertarget{p-129}{}%
Use \hyperref[cor-5-3]{Corollary~\ref{cor-5-3}} with \(f= n_A, g = 1_A, h = (-1)_A\). So the line bundle%
\begin{equation*}
...
\par
\hypertarget{p-134}{}%
Last time Ricky proved theorem of \hyperref[thm-cube]{cube~\ref{thm-cube}} and \hyperref[thm-square]{square~\ref{thm-square}}. For any line bundle  \(L\) on \(X\), there is a group homomorphism \(\Phi_L\colon X \to \Pic(X)\) via \(x\mapsto T_x^* L\otimes L^{-1}\). Be careful \(T_x^*\) is \(-x\), convention, who knows why.%
...
\hypertarget{p-141}{}%
Let \(X\) be an abelian variety and \(L\) a line bundle, \(L = L(D)\) then TFAE:\leavevmode%
\begin{enumerate}
...
\hypertarget{p-194}{}%
There is a unique line bundle \(\sheaf P\) on \(A\times \hat A\) called the \terminology{Poincaré bundle} such that\leavevmode%
\begin{enumerate}
...
\end{equation*}
%
\item\hypertarget{li-37}{}If \(Z\) is a  scheme with a line bundle \(R\) on \(A\times Z\) satisfying 1., 2., there exists a unique%
\begin{equation*}
f\colon Z\to \hat A
\end{equation*}
s.t.%
...
\hypertarget{p-195}{}%
Let \(f\colon A\to B\) be a homomorphism of abelian varieties. Let \(\sheaf P_A,\sheaf P_B\) be the Poincaré bundles on  \(A\) and \(B\). Consider \(M=    (F\times \id_{\hat B})^* \sheaf P_B\) on \(A\times \hat B\), then\leavevmode%
\begin{enumerate}
...
\hypertarget{p-255}{}%
Let \(A\) be an abelian variety of dimension \(g\) then\leavevmode%
\begin{enumerate}
...
\end{definition}
\hypertarget{p-283}{}%
So we need to understand étale covers of abelian varieties. Following \hyperlink{bib-vandergeer-moonen}{[70]}:%
...
\hypertarget{p-346}{}%
Let \((D, ')\) be an Albert algebra, let \(K\) be the center of \(D\) and \(K_0\) the subfield fixed by \('\). Then we have the following classification\leavevmode%
\begin{enumerate}
...
\item\hypertarget{li-58}{}Type III: \(D\) is a quaternion algebra over \(K = K_0\) a totally real field, that is ramified at all infinite places and \('\) is the standard quaternion algebra conjugation as above.%
...
\hypertarget{p-352}{}%
Fix \(\alpha \in \End^0(A)\) and \(l\ne \characteristic(k)\) odd. Applying \hyperref[prop-milne-av-13-6]{Proposition~\ref{prop-milne-av-13-6}} we see that \(\lambda \alpha = \phi_{\sheaf L}\) for some \(\sheaf L\) iff \(e^{\lambda\alpha}_l\) is skew-symmetric, but we also have%
\begin{equation*}
...
\item\hypertarget{li-61}{}If \(\alpha\) is  a root of \(f_X\), then \(\bar \alpha\) with the same multiplicity. In particular if \(\alpha = \pm \sqrt q\) then it occurs with even multiplicity.%
\end{enumerate}
%
\end{theorem}
\hypertarget{p-366}{}%
...
\hypertarget{p-380}{}%
Let \(A, B\) be abelian varieties over \(\FF_q\) and \(P_A, P_B\) as above\leavevmode%
\begin{enumerate}
...
\hypertarget{p-383}{}%
Recall: The isogeny category, \hyperref[thm-poincare-reducibility]{Theorem~\ref{thm-poincare-reducibility}}, \hyperref[cor-ab-var-product-simple]{Corollary~\ref{cor-ab-var-product-simple}}. So we have  a category \(\cat{Isog}\) of abelian varieties with%
\begin{equation*}
...
\par
\hypertarget{p-384}{}%
\(\cat{Isog}\) is a semisimple abelian category. The simples are simple abelian varieties.\leavevmode%
\begin{enumerate}
\item\hypertarget{li-71}{}Decomposition up to isogeny into a product of simple abelian varieties is unique.%
\item\hypertarget{li-72}{}If \(A\) is simple \(\End A \otimes \QQ\) is a division algebra over \(\QQ\). Reason: If \(A\) is simple in an abelian category, if \(\End A \supseteq k\) a field implies it's a division algebra.%
\end{enumerate}
%
\end{subsectionptx}
%
...
\end{equation*}
the endomorphism of \(T_l u\) maps \(T_l A\) to \(\bigcap_{i=1}^\infty U_{n_i} = W\cap T_l A\) passing to \(\QQ_l\)-coefficients, note \(\QQ_l(W\cap T_l A) = \QQ_l(l^n(W\cap T_l A)) = W\) so \(\image(V_l(u)) =W\).%
\end{proof}
\hypertarget{p-395}{}%
...
\item\hypertarget{li-89}{}Find an abelian variety \(A/\CC\) of type \((L, \Phi)\).%
...
\begin{introduction}{}%
\hypertarget{p-456}{}%
These are notes for BUNTES Spring 2018, the topic is Dessins d'Enfants, they were last updated November 19, 2018. For more details see \href{http://math.bu.edu/people/angusmca/buntes/spring2018.html}{the webpage}. These notes are by Alex, feel free to email me at \href{mailto:alex.j.best@gmail.com}{\nolinkurl{alex.j.best@gmail.com}} to report typos/suggest improvements, I'll be forever grateful.%
\end{introduction}%
%
%
\typeout{************************************************}
...
\hypertarget{p-478}{}%
Compute the Dessins for the following Belyi morphisms\leavevmode%
\begin{enumerate}
...
\end{inlineexercise}
\hypertarget{p-510}{}%
In particular there are at least two distinct ways of thinking about genus for Riemann surfaces \(R\)\leavevmode%
\begin{enumerate}
...
\begin{enumerate}
\item\hypertarget{li-103}{}Analysis: Compact connected riemann surfaces.%
\item\hypertarget{li-104}{}Algebra: Field extensions \(K/\CC\) where \(K\) is finitely generated of transcendence degree 1 over \(\CC\).%
\item\hypertarget{li-105}{}Geometry: Complete nonsingular irreducible algebraic curves in \(\PP^n\).%
\end{enumerate}
%
\par
\hypertarget{p-524}{}%
...
\item\hypertarget{li-108}{}Automorphisms of Riemann surfaces%
\end{enumerate}
%
...
\hypertarget{p-597}{}%
Let \(S \) be a compact riemann surface, then the following are equivalent.\leavevmode%
\begin{enumerate}
...
\item\hypertarget{li-120}{}For an equivalence class \((S,f) = (S_F, R(x,y))\) of ramified covers of \(\PP^1\) set \((S,f)^\sigma = (S^\sigma, f^\sigma) = (S_{F^\sigma}, R^\sigma (x,y))\).%
\end{enumerate}
%
\end{definition}
...
\hypertarget{p-630}{}%
For a compact Riemann surface \(S\) the following are equivalent\leavevmode%
\begin{enumerate}
\item\hypertarget{li-140}{}\(S\) is defined over \(\overline \QQ\).%
\item\hypertarget{li-141}{}\(\{S^\sigma\}_{\sigma \in \operatorname{Gal}(\CC)}\) contains only finitely many isomorphism classes of Riemann surfaces.%
\end{enumerate}
%
\end{proposition}
...
\hypertarget{p-637}{}%
A dessin is  a pair \((X,D)\) where \(X\) is an oriented compact topological surface and \(D\subset X\) is a finite graph:\leavevmode%
\begin{enumerate}
...
\hypertarget{p-688}{}%
The genus 0 case follows from the fact that the \(f\) is a rational function (and in fact the error term is \(O(1)\)) (exercise). For the general case we need the theory of log heights on curves. From this we require the following\leavevmode%
\begin{itemize}[label=\textbullet]
...
\item\hypertarget{li-164}{}On Marshall Hall's Conjecture and Gaps Between Integer Points on Mordell Elliptic Curves - Ryan D'Mello%
...
\item\hypertarget{li-169}{}Algorithms and differential relations for Belyi functions - Mark van Hoeij, Raimundas Vidunas.%
...
\item\hypertarget{li-172}{}A note on integral points on elliptic curves - Mark Watkins%
\item\hypertarget{li-173}{}On Hall’s conjecture - Andrej Dujella (more recent progress)%
\item\hypertarget{li-174}{}Hecke Groups, Dessins d'Enfants and the Archimedean Solids -  Yang-Hui He, and James Read%
...
\typeout{************************************************}
%
\begin{sectionptx}{Three Short Stories about Belyi's theorem (Ricky)}{}{Three Short Stories about Belyi's theorem (Ricky)}{}{}\label{sec-three-short-stories-belyi}
\begin{introduction}{}%
...
Outline:\leavevmode%
\begin{enumerate}
\item\hypertarget{li-178}{}Background, isogeny graphs, applications.%
\item\hypertarget{li-179}{}Supersingular isogeny graph cryptography (candidate for post-quantum cryptography).%
...
\item\hypertarget{li-185}{}every finite subgoup of \(E(\overline k )\) is the kernel of a separable isogeny from \(E\), unique up to isomorphism.%
\item\hypertarget{li-186}{}Every \(n\)-isogeny \(\phi\colon E \to E'\) has  a dual isogeny \(\hat \phi \colon E' \to E\) such that%
\begin{equation*}
...
\leavevmode%
\begin{enumerate}
\item\hypertarget{li-188}{}\(\phi_l\) is symmetric in \(x,y\) and has a degree \(l+1\) in both variables.%
...
\hypertarget{p-799}{}%
Let \(E/k = \FF_{p^n}\) be an elliptic curve, TFAE\leavevmode%
\begin{enumerate}
...
\par
\hypertarget{p-811}{}%
Let \(\phi\colon E\to E'\) be an \(l\)-isogeny between two elliptic curves with CM by \(\ints = \ZZ+\tau\ZZ\), \(\ints ' = \ZZ+\tau'\ZZ\). Then \(\hat\phi \tau' \phi \in \End(E) \implies l\tau ' \in \ints\). Similarly \(l\tau \in \ints'\). There are 3 cases\leavevmode%
\begin{enumerate}
...
\begin{enumerate}
\item\hypertarget{li-200}{}All vertices in \(V_i\) have the same endomorphism ring \(\ints_i\).%
...
\leavevmode%
\begin{enumerate}
\item\hypertarget{li-210}{}Preimage resistant iff when given \(j\) it is difficult to compute a positive integer \(e\) and an isogeny \(\phi\colon E_{j_0} \to E_j\) with degree \(l^e\).%
...
\leavevmode%
\begin{enumerate}
\item\hypertarget{li-212}{}(Decisional supersingular isogeny problem) Given \(E, (P_A,Q_A)\) a basis for \(l_A^{e_A}\) torsion, let \(E_A\) be another curve, is \(E_A\) \(l_A^{e_A}\) isogenous to \(E\)?%
\item\hypertarget{li-213}{}(Computational supersingular isogeny problem) Let \(\phi_A \colon E \to E_A\) be an isogeny with a kernel of the form \(\langle m_AP_A + n_AQ_A \rangle\). Given \(E_A\) and \(\phi_A(P_B)\) \(\phi_A(Q_B)\), find \(R_A\). \(p^{1/4}\) classical, \(p^{1/6}\) quantum.%
...
\item\hypertarget{li-219}{}Choosing a s.s. e.c. with the right group order, Broker's algorithm.%
...
\item\hypertarget{li-222}{}Check the order of \((l_B^{e_B} f)^2 \cdot P\). If its \(l_A^{e_A}\) set \(P_A = P\). Otherwise repeat from 1.%
\item\hypertarget{li-223}{}Do the same with \(Q_A = Q\).%
\item\hypertarget{li-224}{}Check independence by seeing if \(e(P_A,Q_A) \) has the right order, so that it is in \(E\lb l_A^{e_A} \rb\) torsion.%
\end{enumerate}
%
...
\item\hypertarget{li-226}{}(Computing smooth degree isogenies) Decompose the \(l_A^{e_A}\) isogeny into \(e_A\) different \(l_A\)-isogenies, \(\phi_i\colon E_i \to E_{i+1}\) the kernel of \(\phi_i\) is \(\langle l_A^{e_A - i - 1} R_A\rangle\). Vélu's formula runs in \(O(l)\) for \(l\)-isogeny.%
\end{enumerate}
%
\end{subsectionptx}
\end{sectionptx}
...
\begin{enumerate}
\item\hypertarget{li-227}{}Computational Problems in Supersingular Elliptic Curve Isogenies - Steven D. Galbraith and Frederik Vercauteren \url{https://www.esat.kuleuven.be/cosic/publications/article-2842.pdf}%
\item\hypertarget{li-228}{}Computing Isogenies Between Abelian Varieties - David Lubicz Damien Robert \url{https://perso.univ-rennes1.fr/david.lubicz/articles/isogenies.pdf}%
\item\hypertarget{li-229}{}Toric forms of elliptic curves and their arithmetic - Wouter Castryck and Frederik Vercauteren \url{https://homes.esat.kuleuven.be/\~fvercaut/papers/ec_forms.pdf}%
\item\hypertarget{li-230}{}Isogenies of Elliptic Curves: A Computational Approach - Daniel Shumow \url{https://www.sagemath.org/files/thesis/shumow-thesis-2009.pdf}%
\item\hypertarget{li-231}{}Hard and Easy Problems for Supersingular Isogeny Graphs - Christophe Petit and Kristin Lauter \url{https://eprint.iacr.org/2017/962.pdf}%
...
\item\hypertarget{li-233}{}COMPUTING ISOGENIES BETWEEN SUPERSINGULAR ELLIPTIC CURVES OVER Fp CHRISTINA DELFS AND STEVEN D. GALBRAITH \url{http://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.740.6509\&rep=rep1\&type=pdf}%
\end{enumerate}
%
\end{subsubsectionptx}
\end{subsectionptx}
...
\item\hypertarget{li-235}{}Hard and Easy Problems for Supersingular Isogeny Graphs - Christophe Petit and Kristin Lauter \url{https://eprint.iacr.org/2017/962.pdf}%
\end{enumerate}
%
\end{introduction}%
%
...
\hypertarget{p-952}{}%
Let \(\ints \subseteq B\) be an order.\leavevmode%
\begin{enumerate}
\item\hypertarget{li-236}{}The \terminology{genus} \(\operatorname{Gen}(\ints)\) of \(\ints \) is the set of orders in \(B \) connected to \(\ints\).%
\item\hypertarget{li-237}{}The type set \(\operatorname{Typ}(\ints)\) of \(\ints\) is the set of \(\ZZ\)-algebra isomorphism classes of orders in \(\operatorname{Gen} (\ints)\).%
\end{enumerate}
%
\end{definition}
...
\leavevmode%
\begin{enumerate}
\item\hypertarget{li-238}{}Any two maximal orders in \(B\) are connected.%
\item\hypertarget{li-239}{}In particular there are only finitely many conjugacy classes of maximal orders in \(B\).%
\end{enumerate}
%
\end{remark}
...
\item\hypertarget{li-247}{}For \(J \in \{J_s,J_t\}\) and corresponding \(E \in \{E_s,E_t\}\) and \(e\in \{e_s, e_t\}\) compute \(J_i = \ints_0 p^2 + \ints_0 l^i\) for \(i = 0,\ldots, e\). For \(i = 0,\ldots, e\) compute \(K_i \in \lb J_i \rb_L\) with powersmooth norm. Translate \(K_i\) into an isogeny%
\begin{equation*}
\phi\colon E_0 \to E_i
\end{equation*}
...
\begin{enumerate}
\item\hypertarget{li-251}{}Analyse local \(p\)-adic galois action on \(p\)-torsion of elliptic curves, Serre's open image theorem.%
\begin{equation*}
...
\item\hypertarget{li-253}{}Describe local properties of moduli spaces of abelian varieties which map to moduli spaces of \(p\)-divisible groups which can be described by semilinear algebra (Serre-Tate).%
\item\hypertarget{li-254}{}Explicit local CFT via Lubin-Tate formal groups describing wildly ramified abelian extensions.%
...
\begin{enumerate}
\item\hypertarget{li-260}{}Smooth proj. curve genus 1 with a rational point.%
...
\hypertarget{p-1089}{}%
An elliptic curve is any one of the following\leavevmode%
\begin{enumerate}
\item\hypertarget{li-270}{}Smooth projective curve of genus 1 with  a marked rational point.%
...
\begin{enumerate}
\item\hypertarget{li-276}{}Any morphism of abelian varieties that preserves the identity is a homomorphism.%
...
\typeout{************************************************}
%
\begin{subsectionptx}{Ablelian varieties over \(\CC\)}{}{Ablelian varieties over \(\CC\)}{}{}\label{subsection-79}
...
\hypertarget{p-1125}{}%
A PPAV (principally polarized abelian variety) is a pair \((A,\lambda)\).%
\end{definition}
\hypertarget{p-1126}{}%
...
%
\typeout{************************************************}
\typeout{Section 5.5 Variations of Hodge Structures}
\typeout{************************************************}
%
\begin{sectionptx}{Variations of Hodge Structures}{}{Variations of Hodge Structures}{}{}\label{sec-buntes-vhs}
%
%
\typeout{************************************************}
...
\typeout{************************************************}
%
\begin{subsectionptx}{Review of Hodge Theory}{}{Review of Hodge Theory}{}{}\label{subsection-84}
\hypertarget{p-1152}{}%
...
\begin{enumerate}
\item\hypertarget{li-283}{}To what extent does the HS of \(X\) determine \(X\)? (Torelli problem)%
\item\hypertarget{li-284}{}To what extent can we read off the geometric data of \(X\) from its Hodge structure.%
\end{enumerate}
%
\end{subsectionptx}
%
...
\typeout{************************************************}
%
\begin{subsectionptx}{Variations of Hodge structures:}{}{Variations of Hodge structures:}{}{}\label{subsection-85}
\begin{introduction}{}%
\hypertarget{p-1156}{}%
...
\typeout{************************************************}
%
\begin{subsubsectionptx}{Hodge theory for curves}{}{Hodge theory for curves}{}{}\label{subsubsection-3}
\hypertarget{p-1158}{}%
...
\leavevmode%
\begin{enumerate}
\item\hypertarget{li-285}{}\(\phi\colon B\to {\mathrm{Gr}}\) mapping \(b \mapsto F^k(X_b) \subseteq H_\CC\) is holomorphic.%
\item\hypertarget{li-286}{}In terms of identifying the tangent space of the grassmanian to the hom set, the image under%
\begin{equation*}
\diff \phi_k = \delta_k
\end{equation*}
...
\begin{definition}{}{definition-114}%
\hypertarget{p-1167}{}%
...
\typeout{************************************************}
%
\begin{sectionptx}{Moduli of linearized \(\CC\)-structures (RICKY)}{}{Moduli of linearized \(\CC\)-structures (RICKY)}{}{}\label{sec-buntes-ricky2}
%
%
\typeout{************************************************}
...
\typeout{************************************************}
%
\begin{subsectionptx}{Motivation: Period morphisms}{}{Motivation: Period morphisms}{}{}\label{subsection-86}
\hypertarget{p-1176}{}%
...
\typeout{************************************************}
%
\begin{subsectionptx}{Moduli of Hodge structures}{}{Moduli of Hodge structures}{}{}\label{subsection-87}
\hypertarget{p-1180}{}%
...
\end{lemma}
\begin{proof}\hypertarget{proof-117}{}
\hypertarget{p-1182}{}%
...
%
\end{lemma}
\begin{proof}\hypertarget{proof-118}{}
\hypertarget{p-1190}{}%
...
%
\end{lemma}
\begin{proof}\hypertarget{proof-119}{}
\hypertarget{p-1192}{}%
see notes.%
\end{proof}
\hypertarget{p-1193}{}%
...
\typeout{************************************************}
%
\begin{subsectionptx}{Geometric conditions and chill (on VHS)}{}{Geometric conditions and chill (on VHS)}{}{}\label{subsection-88}
\hypertarget{p-1199}{}%
...
\begin{definition}{}{definition-115}%
\hypertarget{p-1201}{}%
Let \(G\) be a real algebraic group with involution \(\sigma\). The \terminology{real form} associated to \(\sigma\) is%
\begin{equation*}
...
\end{example}
\begin{definition}{}{definition-116}%
\hypertarget{p-1203}{}%
...
\end{lemma}
\begin{proof}\hypertarget{proof-120}{}
\hypertarget{p-1207}{}%
...
\bibitem[39]{bib-hulsbergen}\hypertarget{bib-hulsbergen}{}Hulsbergen, Wilfred WJ. \textit{Conjectures in arithmetic algebraic geometry}. Braunschweig: Vieweg, 1992.
\bibitem[40]{bib-jannsen}\hypertarget{bib-jannsen}{}Jannsen, Uwe. \textit{Continuous Étale Cohomology}. Mathematische Annalen 280.2 (1988): 207-246.
\bibitem[41]{bib-katz-serre-tate}\hypertarget{bib-katz-serre-tate}{}Katz, Nicholas. \textit{Serre-Tate local moduli}. In Surfaces algébriques, pp. 138-202. Springer, Berlin, Heidelberg, 1981.
\bibitem[42]{bib-kedlaya-counting}\hypertarget{bib-kedlaya-counting}{}Kedlaya, Kiran S. \textit{Counting points on hyperelliptic curves using Monsky-Washnitzer cohomology}, J. Ramanujan Math. Soc. 16 (2001), no. 4, 323-338; errata, ibid. 18 (2003), 417--418.
\bibitem[43]{bib-lang-algebra}\hypertarget{bib-lang-algebra}{}Lang, Serge,  \textit{Algebra}, Graduate Texts in Mathematics 1.211 (2002): ALL-ALL.
\bibitem[44]{bib-lee-smooth}\hypertarget{bib-lee-smooth}{}Lee, John M. \textit{Smooth manifolds}. Springer, New York, NY, 2003. 9780387954486.
\bibitem[45]{bib-le-gall}\hypertarget{bib-le-gall}{}Le Gall, François. \textit{Faster algorithms for rectangular matrix multiplication}. In Foundations of Computer Science (FOCS), 2012 IEEE 53rd Annual Symposium on, pp. 514-523. IEEE, 2012.
\bibitem[46]{bib-liu}\hypertarget{bib-liu}{}Qing Liu. \textit{Algebraic geometry and arithmetic curves}. Vol. 6. Oxford University Press, 2002.
\bibitem[47]{bib-matsumura}\hypertarget{bib-matsumura}{}Matsumura, Hideyuki. \textit{Commutative ring theory}. Vol. 8. Cambridge university press, 1989.
\bibitem[48]{bib-matsumura-algebra}\hypertarget{bib-matsumura-algebra}{}Matsumura, Hideyuki. \textit{Commutative algebra}. Vol. 120. New York: WA Benjamin, 1970.
\bibitem[49]{bib-mazur-stein-tate}\hypertarget{bib-mazur-stein-tate}{}Mazur, Barry, William Stein, and John Tate. \textit{Computation of p-adic heights and log convergence.} Doc. Math (2006): 577-614.
\bibitem[50]{bib-mtt}\hypertarget{bib-mtt}{}Mazur, Barry, John Tate, and Jeremy Teitelbaum. \textit{On p-adic analogues of the conjectures of Birch and Swinnerton-Dyer}. Inventiones mathematicae 84, no. 1 (1986): 1-48.
\bibitem[51]{bib-milne-etale}\hypertarget{bib-milne-etale}{}Milne, James S. \textit{Etale cohomology} (PMS-33). Vol. 33. Princeton university press, 2016.
\bibitem[52]{bib-minzlaff}\hypertarget{bib-minzlaff}{}Minzlaff, Moritz. \textit{Computing zeta functions of superelliptic curves in larger characteristic}. Mathematics in Computer Science 3.2 (2010): 209-224.
\bibitem[53]{bib-mumford-abvar}\hypertarget{bib-mumford-abvar}{}Mumford, David. \textit{Abelian varieties}. Vol. 5. Oxford University Press, USA, 1974.
\bibitem[54]{bib-narkiewicz}\hypertarget{bib-narkiewicz}{}Narkiewicz, Wladyslaw  \textit{Elementary and Analytic Theory of Algebraic Numbers}. Springer, 2004.
\bibitem[55]{bib-polishchuck}\hypertarget{bib-polishchuck}{}Polishchuk, Alexander. \textit{Abelian varieties, theta functions and the Fourier transform}. Vol. 153. Cambridge University Press, 2003.
\bibitem[56]{bib-rolshausen}\hypertarget{bib-rolshausen}{}Rolshausen, Klaus, and Norbert Schappacher.  \textit{On the second K-group of an elliptic curve.}  Journal für die reine und angewandte Mathematik 495 (1998): 61-77.
\bibitem[57]{bib-sage}\hypertarget{bib-sage}{}\textit{SageMath, the Sage Mathematics Software System (Version 8.1.0)}, The Sage Developers, 2017, \url{http://www.sagemath.org}.
\bibitem[58]{bib-serre-course}\hypertarget{bib-serre-course}{}Serre, Jean-Pierre. \textit{A course in arithmetic}. Vol. 7. Springer Science \& Business Media, 2012.
\bibitem[59]{bib-serre-gal-coh}\hypertarget{bib-serre-gal-coh}{}Serre, Jean-Pierre. \textit{Galois cohomology}. Springer Science \& Business Media, 2013.
\bibitem[60]{bib-serre-local-alg}\hypertarget{bib-serre-local-alg}{}Serre, Jean-Pierre. \textit{Local algebra}. Springer Science \& Business Media, 2000.
\bibitem[61]{bib-serre-mordell-weil}\hypertarget{bib-serre-mordell-weil}{}Serre, Jean-Pierre, Martin Brown, and Michel Waldschmidt. \textit{Lectures on the Mordell-Weil theorem}. Vol. 2. Braunschweig: Vieweg, 1990.
\bibitem[62]{bib-shafarevich-remizov}\hypertarget{bib-shafarevich-remizov}{}Shafarevich, Igor R., and Alexey O. Remizov. \textit{Linear algebra and geometry}. Springer Science \& Business Media, 2012.
\bibitem[63]{bib-silverman}\hypertarget{bib-silverman}{}Silverman, Joseph H. \textit{The arithmetic of elliptic curves}. Vol. 106. Springer Science \& Business Media, 2009.
\bibitem[64]{bib-silverman-advanced}\hypertarget{bib-silverman-advanced}{}Silverman, Joseph H. \textit{Advanced topics in the arithmetic of elliptic curves}. Vol. 151. Springer Science \& Business Media, 2013.
\bibitem[65]{bib-stacks}\hypertarget{bib-stacks}{}Stacks Project Authors, \textit{The Stacks Project}, \url{https://stacks.math.columbia.edu}.
\bibitem[66]{bib-sutherland}\hypertarget{bib-sutherland}{}Sutherland, Andrew. \textit{Isogeny volcanoes}. The Open Book Series 1, no. 1 (2013): 507-530. \url{https://msp.org/obs/2013/1-1/obs-v1-n1-p25-s.pdf}.
\bibitem[67]{bib-tamme}\hypertarget{bib-tamme}{}Tamme, Günter. \textit{Introduction to étale cohomology}. Springer Science \& Business Media, 2012.
\bibitem[68]{bib-tuitman-map-to-p1-1}\hypertarget{bib-tuitman-map-to-p1-1}{}Tuitman, Jan. \textit{Counting points on curves using a map to \(\PP^1\)}. Mathematics of Computation 85.298 (2016): 961-981.
\bibitem[69]{bib-tuitman-map-to-p1-2}\hypertarget{bib-tuitman-map-to-p1-2}{}Tuitman, Jan. \textit{Counting points on curves using a map to \(\PP^1\), II}. Finite Fields and Their Applications 45 (2017): 301-322.
\bibitem[70]{bib-vandergeer-moonen}\hypertarget{bib-vandergeer-moonen}{}van der Geer, G., Moonen, B. \textit{Abelian Varieties}, from \url{https://www.math.ru.nl/\~bmoonen/research.html\#bookabvar}.
\bibitem[71]{bib-velu}\hypertarget{bib-velu}{}Vélu, Jacques. \textit{Isogénies entre courbes elliptiques}. CR Acad. Sci. Paris, Séries A 273 (1971): 305-347.
\bibitem[72]{bib-voight-quat}\hypertarget{bib-voight-quat}{}Voight, John. \textit{Quaternion Algebras}. \url{http://quatalg.org}
\bibitem[73]{bib-waterhouse}\hypertarget{bib-waterhouse}{}Waterhouse, William C.  \textit{Profinite groups are Galois groups}. Proceedings of the American Mathematical Society 42.2 (1974): 639-640.
\bibitem[74]{bib-weng-nakamura}\hypertarget{bib-weng-nakamura}{}Weng, L., and Nakamura, I. \textit{Arithmetic geometry and number theory}, World Scientific, 2006.
\bibitem[75]{bib-zag-ram}\hypertarget{bib-zag-ram}{}Zagier, Don.  \textit{The Bloch-Wigner-Ramakrishnan polylogarithm function.} Mathematische Annalen 286, no. 1 (1990): 613-624.
\bibitem[76]{bib-pug}\hypertarget{bib-pug}{}\textit{Pug Template Engine} see \href{http://pug-lang.com/}{pug-lang.com}.
\end{thebibliography}
