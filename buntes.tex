...
\tcbset{ paragraphsstyle/.style={size=minimal, boxrule=-0.3pt, before skip=3ex, frame empty, colback=white, colbacktitle=white, coltitle=black, fonttitle=\normalfont\bfseries, attach title to upper, after title={\space}} }
...
\item\hypertarget{item-nonsing-pt}{}\(E\) is nonsingular of \hyperref[sec-deuring]{genus} 1 with a distinguished point \(P_0\).%
...
\end{equation*}
There exists an open subset \(U\) such that the base change \(X \times_Y U\) is isomorphic with \(\coprod U\) of several copies of \(U\) in a \hyperref[sec-deuring]{compatible} way with the map to \(U\).%
\par
\hypertarget{p-268}{}%
...
\end{align*}
will state many properties later, but for now. \(e_m\) is \hyperref[sec-deuring]{compatible}:%
\begin{equation*}
...
\item\hypertarget{li-47}{}\(e_m\) is \hyperref[sec-deuring]{compatible}%
\begin{equation*}
...
\hypertarget{p-477}{}%
We have a faithful action of \(\absgal{\QQ}\) on the set of \hyperref[def-dessin-denfant]{dessins} of any fixed \hyperref[sec-deuring]{genus}.%
\end{theorem}
%
%
\typeout{************************************************}
...
\hypertarget{p-509}{}%
The \hyperref[sec-deuring]{genus} is invariant under changing triangulation.%
\end{inlineexercise}
\hypertarget{p-510}{}%
In particular there are at least two distinct ways of thinking about \hyperref[sec-deuring]{genus} for \hyperref[def-top-riem-surface]{Riemann surfaces} \(R\)\leavevmode%
\begin{enumerate}
...
\par
\hypertarget{p-511}{}%
Goal: given \(R\) calculate \hyperref[sec-deuring]{genus}%
\begin{equation*}
y^2 = (x+1)(x-1)(x+2)(x-2)
\end{equation*}
so in an ad hoc way%
...
\hypertarget{p-533}{}%
There are no non-constant morphisms from a sphere to a surface of \hyperref[sec-deuring]{genus} \(\gt 0\).%
\end{corollary}
...
x^3 y + y^3 z + z^3 x = 0
\end{equation*}
has 168 automorphisms and is \hyperref[sec-deuring]{genus} 3.%
\end{inlineexercise}
\end{paragraphs}%
%
%
...
\end{proof}
\hypertarget{p-570}{}%
Say \(\Sigma\), \(g\ge 2\), \(G\subseteq \Aut(\Sigma)\). Let \(\bar g\) be the \hyperref[sec-deuring]{genus} of \(\Sigma /G\)%
\begin{equation*}
...
\end{equation*}
compute \hyperref[sec-deuring]{genus} of \(\HH/\Gamma(n)\).%
\end{inlineexercise}
\end{paragraphs}%
%
%
...
\hypertarget{p-573}{}%
\(g =0\). Uniformization tells us that up to isomorphisms all \hyperref[def-top-riem-surface]{Riemann surfaces} of \hyperref[sec-deuring]{genus} \(0\) are \(\PP^1\) hence the moduli space \(\mathcal M _0  = \{\text{pt}\}\).%
\par
\hypertarget{p-574}{}%
\(g = 1\). Uniformization tells us that each \hyperref[def-top-riem-surface]{Riemann surface} of \hyperref[sec-deuring]{genus} \(1\) is a torus and can be written as \(\CC/ \omega_1\ZZ+ \omega_2\ZZ \to \CC/(\ZZ \oplus \tau \ZZ)\), with \(\tau = \pm \omega_1/\omega_2\).%
...
(x,y) \to x
\end{equation*}
\(S_F\) has \hyperref[sec-deuring]{genus} 0. \(S_F \to \PP^1\) is of degree 3 and \hyperref[def-dess-ramified]{ramified} at most over \(0, \frac{-27}{4}, \infty\). \(\operatorname{Mon}(x)  \simeq \Sigma_3\) so not a normal covering. Normalization of \((S_F, x)\) is \(S_{\tilde F} , \tilde x)\) where%
\begin{equation*}
...
\par
\hypertarget{p-660}{}%
The \(G_\QQ\) action is faithful on \hyperref[def-dessin-denfant]{dessins} of \hyperref[sec-deuring]{genus} \(g\).%
...
\hypertarget{p-666}{}%
Let \(C\) be an algebraic curve of \hyperref[sec-deuring]{genus} \(\ge 2\) over a number field \(K\). Then \(C(K)\) is finite.%
\end{theorem}
\hypertarget{p-667}{}%
...
\hypertarget{p-685}{}%
If \(C\)  has \hyperref[sec-deuring]{genus} 0 or 1, no such \(f\) can exist (hint: Riemann-Hurwitz).%
\end{inlineexercise}
\end{paragraphs}%
...
\hypertarget{p-688}{}%
The \hyperref[sec-deuring]{genus} 0 case follows from the fact that the \(f\) is a rational function (and in fact the error term is \(O(1)\)) (exercise). For the general case we need the theory of log heights on curves. From this we require the following\leavevmode%
\begin{itemize}[label=\textbullet]
...
\hypertarget{p-693}{}%
Let \(C\) be a given curve of \hyperref[sec-deuring]{genus} \(g \ge 2\) Belyi's theorem gives a function%
\begin{equation*}
f\colon C \to \PP^1
\end{equation*}
...
\hypertarget{p-756}{}%
\(\PP^1 \smallsetminus \{0,1,\infty\}\) is the moduli space of \hyperref[sec-deuring]{genus} 0 curves with four (ordered) marked points.%
\begin{equation*}
...
\hypertarget{p-757}{}%
Let \(\mathcal M_{g,n}\) be the moduli space of \hyperref[sec-deuring]{genus} \(g\) curves with \(n\) (ordered) marked points (then \(\mathcal M_{g,\lb n\rb}\) is the same for unordered points). If \(n\) is large enough relative to \(g\) then \(\mathcal M_{g,n}\) will be a scheme (but the unordered version will not).%
\end{definition}
...
E_1 \times E_2
\end{equation*}
or \(J(C)\) for \(C\) of \hyperref[sec-deuring]{genus} 2.%
\par
\hypertarget{p-766}{}%
Case 1:%
\par
...
\hypertarget{p-787}{}%
An \terminology{elliptic curve} \(E/k\) is a smooth projective curve of \hyperref[sec-deuring]{genus} 1 together with a point \(\infty \in E(k)\).%
\end{definition}
\hypertarget{p-788}{}%
...
\typeout{************************************************}
%
\section[{The Deuring Correspondence (Maria Ines)}]{The Deuring Correspondence (Maria Ines)}\label{sec-deuring}
\begin{introduction}{}%
\hypertarget{p-935}{}%
References:%
\par
...
\typeout{************************************************}
%
\subsection[{Background: Ideals and Ideal classes}]{Background: Ideals and Ideal classes}\label{subsection-75}
\hypertarget{p-937}{}%
Let \(B/\QQ\) be a quaternion algebra and  \(\ints \subseteq B\) be an \hyperref[def-order-quaternion]{order}. If \(I \subseteq B \) is a  lattice, we can define \(\ints_L (I) = \{ \alpha\in  B :  \alpha I  \subseteq I \}\). This is an \hyperref[def-order-quaternion]{order} , it's the left \hyperref[def-order-quaternion]{order} of  \(I\) similarly can define \(\ints_R (I) \).%
...
\end{definition}
\begin{definition}{}{definition-85}%
\hypertarget{p-939}{}%
...
\end{equation*}
with both products \hyperref[sec-deuring]{compatible}%
\end{definition}
...
\end{proposition}
\begin{definition}{}{definition-86}%
\hypertarget{p-941}{}%
...
\end{fact}
\begin{definition}{}{definition-87}%
\hypertarget{p-943}{}%
...
\end{definition}
\begin{definition}{}{definition-89}%
\hypertarget{p-945}{}%
...
\typeout{************************************************}
%
\subsection[{The Deuring Correspondence}]{The Deuring Correspondence}\label{subsection-76}
\hypertarget{p-955}{}%
...
\typeout{************************************************}
%
\subsection[{Applications to SIG crypto}]{Applications to SIG crypto}\label{subsection-77}
